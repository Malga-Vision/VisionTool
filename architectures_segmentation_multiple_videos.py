...
class unet_multiple_videos():
    ...
    def __init__(self,architecture, backbone,image_net,single_labels,colors,address,annotation_file,image_folder,annotation_folder,bodyparts,BATCH_SIZE = 5,train_flag=1,annotation_assistance = 0,lr = 0.001,loss = "Weighted Categorical_cross_entropy",markersize=13):
        ...
        self.name_video_list = list()

        for i in range(0, self.number_videos):
            ind = self.pref[2+i].rfind(os.sep)
            self.name_video_list.append(self.pref[2+i][ind+1:-1])
    ...
    def data_preparing(self):
        ...
        for video_index in range (0,self.number_videos):
            ...
            self.annotation_file = os.path.join(self.address,self.annotations[video_index])
            ...
            self.image_folder = os.path.join(self.address,self.frames[video_index])
            ...
            self.IMG_WIDTH = 288   # for linkNet compatibility
            self.IMG_HEIGHT = 288  # for linkNet compatibility
            ...
            X_train = np.zeros((len(self.annotated), self.IMG_HEIGHT, self.IMG_WIDTH, self.IMG_CHANNELS), dtype=np.uint8)
            ...
            for l in range(0, len(self.annotated)):
                ...
                for j in range(0, self.num_bodyparts):
                    ...
                    points = np.asarray([self.dataFrame[self.dataFrame.columns[j * 2]].values[self.annotated[l]],
                                         self.dataFrame[self.dataFrame.columns[j * 2 + 1]].values[self.annotated[l]]],
                                        dtype=float)
                ...
                mask_ = mask_ / 255.0
                # needed to solve bug for only 1 label present!
                if len(np.shape(mask_)) == 2:
                    mask_copy = np.zeros((np.shape(mask_)[0], np.shape(mask_)[1], 1))
                    mask_copy[:, :, 0] = mask_
                    mask_ = mask_copy
            ...
            try:
                ...
                wx.MessageBox('two or more labels are overlapping in video' + self.name_video_list[video_index] + '\n '
        ...
        for video_index in range(0, self.number_videos):
            ...
            try:
                ...
                if not os.path.isfile(self.annotation_file):
                    wx.MessageBox('Are you sure you extracted the frames for all the videos?\n '
                                  'Please, extract frames of videos before performing analysis'
                                  , 'Error!', wx.OK | wx.ICON_ERROR)
            ...
            OUTPUT = os.path.join(self.address,self.name_video_list[video_index] + '_prediction')
            ...
            address_single_labels = os.path.join(self.address, self.name_video_list[video_index] + '_Single_labels')
